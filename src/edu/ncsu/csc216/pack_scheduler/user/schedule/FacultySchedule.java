...
public class FacultySchedule {
	...
	public boolean addCourseToSchedule(Course course) {
		for (int i = 0; i < schedule.size(); i++) {
			if (schedule.get(i).isDuplicate(course)) { // if duplicate
				throw new IllegalArgumentException("Already assigned " + course.getName());
			}
			try {
				schedule.get(i).checkConflict(course); // if conflict
			} catch (ConflictException e) {
				throw new IllegalArgumentException("The course cannot be assigned due to a conflict.");
			}
		}
		if (course.getInstructorId() == null || course.getInstructorId().equals("")) {
	...
	/**
     * Gets the 2D array of students from the CourseRoll stored in the Course indexed at the given index of Schdule.
     * @param i the index to get from
     * @return 2D array of student info for the given course
     */
    public String[][] getStudents(int i) {
        return schedule.get(i).getCourseRoll().getRoll();
    }
